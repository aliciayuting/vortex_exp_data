...
def get_durations(df, start_tag, end_tag, group_by_columns=['node_id'], duration_name='latency'):
    ...
    for group_values, timestamps in grouped:
        ...
        # If there is only one grouping column, `group_values` is still a tuple
        if len(group_by_columns) == 1:
            group_values = group_values[0]  # Unpack the tuple
        # Store results
...
def process_e2e_dataframe(df):
     ...
     sub_component_latencies['e2e_time'] = get_durations(df, 70100, 1000, group_by_columns=['node_id'], duration_name='e2e_time') #mono
...
def process_last_udl_dataframe(df):
     sub_component_latencies = {}
     sub_component_latencies['last_udl_time'] = get_durations(df, 40031, 40030, group_by_columns=['node_id'], duration_name='last_udl_time')
     return sub_component_latencies

def process_bw_udls_dataframe(df):
     sub_component_latencies = {}
     sub_component_latencies['c_udla'] = get_durations(df, 10000, 1000, group_by_columns=['node_id'], duration_name='c_udla')
     sub_component_latencies['c_udlb'] = get_durations(df, 20000, 10100, group_by_columns=['node_id'], duration_name='c_udlb')
     sub_component_latencies['udla_d'] = get_durations(df, 50000, 20100, group_by_columns=['node_id'], duration_name='udla_d')
     sub_component_latencies['udlb_d'] = get_durations(df, 70000, 20100, group_by_columns=['node_id'], duration_name='udlb_d')
     sub_component_latencies['udld_e'] = get_durations(df, 40000, 30100, group_by_columns=['node_id'], duration_name='udld_e')
     return sub_component_latencies

def process_c_mono_dataframe(df):
     sub_component_latencies = {}
     sub_component_latencies['c_mono'] = get_durations(df, 40000, 1000, group_by_columns=['node_id'], duration_name='c_mono')
...
def process_udls_dataframe(df):
     ...
     # sub_component_latencies['udlB'] = get_durations(df, 20031, 20030, group_by_columns=['node_id'], duration_name='udlB')
     # sub_component_latencies['udlD'] = get_durations(df, 50031, 50030, group_by_columns=['node_id'], duration_name='udlD')    
     # sub_component_latencies['udlE'] = get_durations(df, 70031, 70030, group_by_columns=['node_id'], duration_name='udlE')
     
     sub_component_latencies['udlA'] = get_durations(df, 10030, 10000, group_by_columns=['node_id'], duration_name='udlA')
     sub_component_latencies['udlB'] = get_durations(df, 20030, 20000, group_by_columns=['node_id'], duration_name='udlB')
     sub_component_latencies['udlD'] = get_durations(df, 50030, 50000, group_by_columns=['node_id'], duration_name='udlD')    
     sub_component_latencies['udlE'] = get_durations(df, 70010, 70001, group_by_columns=['node_id'], duration_name='udlE')
     return sub_component_latencies
...
def get_batch_size(df):
     ...
     sub_component_batch_sizes["udlA_exec_batch"] = get_batch_size_df(df, 10030, group_by_columns=['node_id'], col_name='udlA_exec_batch')
     sub_component_batch_sizes["udlB_exec_batch"] = get_batch_size_df(df, 20030, group_by_columns=['node_id'], col_name='udlB_exec_batch')
     sub_component_batch_sizes["udlB_emit_batch"] = get_batch_size_df(df, 20100, group_by_columns=['node_id'], col_name='udlB_emit_batch')
     sub_component_batch_sizes["udlD_exec_batch"] = get_batch_size_df(df, 50030, group_by_columns=['node_id'], col_name='udlD_exec_batch')
     sub_component_batch_sizes["udlD_emit_batch"] = get_batch_size_df(df, 50100, group_by_columns=['node_id'], col_name='udlD_emit_batch')
     sub_component_batch_sizes["udlE_exec_batch"] = get_batch_size_df(df, 70030, group_by_columns=['node_id'], col_name='udlE_exec_batch')
     sub_component_batch_sizes["mono_exec_batch"] = get_batch_size_df(df, 70100, group_by_columns=['node_id'], col_name='mono_exec_batch')
