...
def process_cluster_search_udl_dataframe(df):
     ...
     sub_component_latencies['cluster_emb_search_time'] = get_durations_groupby_columns(df, 30021, 30031, group_by_columns=['querybatch_id','cluster_id'], duration_name='cluster_search_time')
...
def process_agg_generate_udl_dataframe(df, old_version=True):
     sub_component_latencies = {}
     ...
     sub_component_latencies['finish_gather_time'] = query_finish_gather_df
     sub_component_latencies['retrieve_doc'] = get_durations(df, 40020, 40021, group_by_column='querybatch_id', duration_name='retrieve_doc_time')
     sub_component_latencies['disk_load_answer_table'] = get_durations(df, 40120, 40121, group_by_column='node_id', duration_name='disk_load_answer_table')
     sub_component_latencies['disk_load_doc'] = get_durations(df, 40220, 40221, group_by_column='node_id', duration_name='disk_load_doc')
