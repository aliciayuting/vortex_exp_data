...
INIT_STEPA_SHARD_INDICES = [3]
INIT_STEPB_SHARD_INDICES = [0,1,2]

STEPA_SHARD_INDICES = [3,7]
STEPB_SHARD_INDICES = [0,1,2,11,12]
...
if __name__ == "__main__":
    ...
    ds = load_dataset('parquet', data_files ={  
                                            ...
                                            })[use_split].select(i for i in range(0, 10000, 1)) 
    ...
    loader = DataLoader(
        ...
        batch_size=BS, 
        shuffle=False, 
        num_workers=32,      # Use multiple workers to prefetch batches in parallel
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if batch_idx <= CUTOFF2 and batch_idx >= WARMUP:
            stepa_next_shard_idx = INIT_STEPA_SHARD_INDICES[(batch_idx) % len(INIT_STEPA_SHARD_INDICES)]
        ...
        if batch_idx <= CUTOFF2 and batch_idx >= WARMUP:
            stepb_next_shard_idx = INIT_STEPB_SHARD_INDICES[(batch_idx) % len(INIT_STEPB_SHARD_INDICES)]
